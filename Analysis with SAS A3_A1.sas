...
proc means data = probit1 mean;
  var age wgt0;
run;
data probit1; set probit1;
   cage = age - 24.2666628;
   cage2=cage**2;
...
*crude estimate (which is confounded);
 proc reg data=probit1 PLOTS=NONE;
  model wgt3 = a3/clb;
   ods output ParameterEstimates = A3_ATE_crude;
 run;

* Simple adjustment in linear regression (GLM because of categorical variables);
proc glm data=probit1 PLOTS=NONE;
class educ location;
model wgt3 = a3 a2 educ smoke allergy cage cage2 location cesarean sex cwgt0 cwgt02  /solution clparm; 
...
proc causaltrt data=probit1 method=regadj ;
class educ location a3;
psmodel a3(ref=FIRST);
model wgt3 = a2 educ smoke allergy cage cage2 location cesarean sex cwgt0 cwgt02  ; 
...
* Build a propensity model using logistic regression with a3 as dependent and 
  location educ smoke cage cage2 allergy a2 cesarean sex cwgt0 cwgt02 as independent variables;
PROC LOGISTIC data=probit1 descending;
  class educ location ;
  model a3 = a2 location educ smoke cage cage2 allergy cesarean sex cwgt0 cwgt02 ; 
...
proc psmatch data =ps1 region=allobs;
  class educ location a2 a3;
  psmodel a3 (Treated='yes')= a2 educ smoke allergy cesarean sex cwgt0 cwgt02 cage cage2 location ;  
...
* Bootstrap;
proc causaltrt data=probit1 ;
bootstrap seed = 462;
class educ location a3;
psmodel a3(ref=FIRST) = a2 educ smoke allergy cesarean sex cwgt0 cwgt02 cage cage2 location ; 
model wgt3;
...
* Double robust;
*bootstrap;
proc causaltrt data=probit1 method=AIPW;
bootstrap seed = 462;
class educ location a3;
psmodel a3(ref=FIRST) = a2 educ smoke allergy cesarean sex cwgt0 cwgt02 cage cage2 location ; 
model wgt3=educ smoke allergy a2 cesarean sex cwgt0 cwgt02 cage cage2 location ; 
...
data ate; set A3_ATE_crude A3_ATE_adjsimple A3_ATE_adjinter A3_ATE_prop_strat A3_ATE_IPW A3_ATE_DR A2_IV;
run;
...
* Then average across the sample;
* This can be done in SAS/STAT 14.2 by proc causaltrt;
proc causaltrt data=probit1 method=regadj att;
class educ location a3;
psmodel a3(ref=FIRST);
model wgt3 = a2 educ smoke allergy cesarean sex cwgt0 cwgt02 cage cage2 location ; 
...
*Propensity weighting;
* Note that the standard errors calculated here also take into account that weights are estimated;
proc causaltrt data=probit1 att;
class educ location a3;
bootstrap seed = 462;
psmodel a3(ref=FIRST) = a2 educ smoke allergy cesarean sex cwgt0 cwgt02 cage cage2 location ; 
model wgt3;
...
* All results;
proc print data=ATE noobs; title"results A3 ATE in subgroup A1=1"; 
var  Estimand Estimate StdErr BTStdErr; run;
proc print data=ATT noobs; title"results A3 ATT in subgroup A1=1";  
var  Estimand Estimate StdErr BTStdErr; run;
